...
export const checkTextWithMultipleFields = (text, prefix, objects, keys) => {
  if (text.startsWith(prefix)) {
    const suffix = text.slice(prefix.length);

    return objects.some((obj) =>
      keys.some((key) => Array.isArray(obj[key]) && obj[key].includes(suffix))
    );
  }
  return false;
};

export const findNameByMatchingFields = (text, prefix, objects, keys) => {
  if (text.startsWith(prefix)) {
    const suffix = text.slice(prefix.length);
    ...
    // 조건을 만족하는 객체가 있다면 name 반환
    return matchedObject ? matchedObject.name : null;
  }
  return null; // 조건을 만족하는 객체가 없을 경우
};
export const imgURL = 'https://kojkkojk.github.io/hamoonrang/dccon/images/';
